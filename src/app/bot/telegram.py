...
def build_start_text(alert_count: int, limit: int = DEFAULT_ALERT_LIMIT) -> str:
    """Build a welcome message for a Telegram user."""
    return (
        "Welcome to Price Tracker.\n"
        f"You currently have {alert_count}/{limit} active alerts.\n"
        "Use /add to create a new alert, /list to review them, or /help for the command guide."
    )
...
class TelegramBotService:
    ...
    def __init__(self, alert_limit: int = DEFAULT_ALERT_LIMIT):
    ...
    def build_registration_summary(self, session, user_id: int) -> str:
        ...
        return build_start_text(active_count, self.alert_limit)
